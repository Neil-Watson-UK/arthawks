/*
 * Install handshake: physical hang is distinct from schedule approval.
 * Public "showing" requires hung_at (backfilled from approved_at for existing lives).
 */
alter table public.matches
  add column if not exists hung_at timestamptz;

comment on column public.matches.hung_at is
  'When the venue marked the work hung on the wall — gates public showing + QR live.';

update public.matches
set hung_at = approved_at
where approved_at is not null
  and hung_at is null;

/* Refresh map RPC: showing = accepted + approved + hung + inside window. */
