...
import { markHungInDb, SchemaNotReadyError } from '$lib/server/rotations-db';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		const result = await markHungInDb(body.match_id, body.wall_label);
		return json(result);
	} catch (err) {
		...
		throw error(500, {
			message: err instanceof Error ? err.message : 'Failed to mark hung on wall'
		});
	}
};
