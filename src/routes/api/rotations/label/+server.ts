...
import { SchemaNotReadyError, updateMatchWallLabel } from '$lib/server/rotations-db';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		await updateMatchWallLabel(body.match_id, body.wall_label ?? '');
		return json({ ok: true });
	} catch (err) {
		...
		throw error(500, {
			message: err instanceof Error ? err.message : 'Failed to update wall label'
		});
	}
};
